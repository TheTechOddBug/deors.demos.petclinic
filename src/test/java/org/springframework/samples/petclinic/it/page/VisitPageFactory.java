...
public class VisitPageFactory {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(VisitPageFactory.class);
	...
	/**
	 * Method used to verify that the page loaded is the one expected. This is
	 * done by matching the Label of the title within the header tags
	 *
	 * @return true if the page contains the title label expected
	 */
	public boolean verifyPage() {

		logger.debug("\t-- -- verifying page with title: \""
				+ titleElement.getText() + "\" contains(\"" + getTitleLabel()
				+ "\")");
		return titleElement.getText().contains(getTitleLabel());
	}

	/**
	 * Method that creates a visit for a given pet at the date received by
	 * parameters and with the description provided
	 *
	 * @param date
	 * @param description
	 */
	public void createVisit(String date, String description) {
		...
		logger.debug("\t-- -- before create visit current driver is:"
		...
		(new WebDriverWait(driver, 5)).until((Predicate<WebDriver>) d -> d
				.getCurrentUrl().startsWith(baseUrl + "/owners/9")
				&& !d.getCurrentUrl().contains("pets/new"));

		logger.debug("\t-- -- after create visit current driver is:"
				+ driver.getCurrentUrl());
